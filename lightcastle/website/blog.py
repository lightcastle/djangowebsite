...
def _remove_wordpress_markup(source):
  ...
  if re.search('\[sourcecode language="(.*)"\]', source): ### this if/else group is to set match variable  so that it doesnt screw up the blog_index page, which also calls this function. could break this out into a separate method. TODO clean this up
    match = re.search('\[sourcecode language="(.*)"\]', source)
    match = match.group(1).lower() #this catches any language names that are camel cased and lowercases them since our code highlighting parser needs all lower case
  else:
